				...
				async.forEachSeries(
					...
					function (schema, callback) {
						var request = new Request(
								...
								"(CONSTRAINT_NAME LIKE 'PK_%' OR CONSTRAINT_NAME LIKE 'PrimaryKey_%') ORDER BY ORDINAL_POSITION",
							function (err, rowCount) {
								callback(err);
							}
						);
...
module.exports = function (cfg, filedir, callback) {
	...
	async.waterfall([
		...
		function (callback) {
			convert_database_schema (cfg, cfg.options.database, filedir, callback);
		}
	], function (err) {
		callback(err);
	});
